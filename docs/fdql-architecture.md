...
Core FDQL language metadata lives in `packages/fdql-core`. Provider language metadata lives on provider dialects. `packages/fdql-language` composes those sources so editor support stays aligned with core syntax and registered providers.
